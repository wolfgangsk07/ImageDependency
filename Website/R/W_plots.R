...
add_lines<-function(drawing,x,y,lwd=1,col="black",lty="solid",groupid=NULL,hideInR=NULL,lend=1){
...
myboxplot<-function(boxdata=NULL){
	...
	for(i in 1:length(boxdata$data)){
		...
		y_box_stats[[i]]<-as.numeric(boxplot(y[[i]],plot=FALSE)$stats)
	}
	...
	for(i in 1:length(boxdata$data)){
		...
		drawing<-add_circle(drawing,x_r,y[[i]],r=plotRadius,col=boxdata$data[[i]]$col,border="black",lwd=0.5,groupid=paste0("boxplotgroup_",i),hideInR="true")
	}
	return(drawing)
}
mykmplot<-function(survivalpackage=NULL){
	...
	#for(j in 1:2){
		...
		for(group_forjs in groups_forjs){
			legend_leftbase=left+innerWidth/5
			...
			for(s in 1:length(groups)){
				...
				#画legend
				drawing<-add_text(drawing,groups[s],x=legend_leftbase,y=legend_ybase,cex=0.75,adj=c(0,0.5),groupid=paste0(group_forjs,s),hideInR=hideInR)
				legend_leftbase=legend_leftbase+getStrLen(groups[s])*dpi*0.75+0.05*dpi
				drawing<-add_lines(drawing,x=legend_leftbase+c(0,surv_tick*4),y=rep(legend_ybase,2),lwd=2.5,col=as.character(cols[s]),groupid=paste0(group_forjs,s),hideInR=hideInR)
				drawing<-add_lines(drawing,x=rep(legend_leftbase+surv_tick*2,2),y=legend_ybase+c(-surv_tick,surv_tick),lwd=1.5,col=as.character(cols[s]),groupid=paste0(group_forjs,s),hideInR=hideInR)
				drawing<-add_lines(drawing,x=legend_leftbase+surv_tick*2+c(surv_tick,-surv_tick),y=rep(legend_ybase,2),lwd=1.5,col=as.character(cols[s]),groupid=paste0(group_forjs,s),hideInR=hideInR)
				...
				for(i in 1:length(surv_event[[groupid]])){
					if(surv_event[[groupid]][i]==1){
						...
						drawing<-add_lines(drawing,x=rep(surv_time[[groupid]][i]/xscale+left,2),y=c(surv_tick,-surv_tick)+tail(ys,1),lwd=1.5,col=as.character(cols[s]),groupid=paste0(group_forjs,s),hideInR=hideInR)
						drawing<-add_lines(drawing,x=surv_time[[groupid]][i]/xscale+left+c(surv_tick,-surv_tick),y=rep(tail(ys,1),2),lwd=1.5,col=as.character(cols[s]),groupid=paste0(group_forjs,s),hideInR=hideInR)
				...
				drawing<-add_lines(drawing,x=xs,y=ys,lwd=2.5,col=as.character(cols[s]),groupid=paste0(group_forjs,s),hideInR=hideInR)
			}
		}
		drawing<-add_text(drawing,paste("p =",round(pval,2)),x=left+innerWidth/4,y=bottom+innerHeight/4,cex=0.5,adj=c(0.5,0.5))
	#}
	return(drawing)
