...
run_cPCA_analysis <- function(tissue_dat, CCLE_dat, tumor_cluster_df, CL_cluster_df, pc_dims=NULL) {
  tumor_clust_avgs <- get_cluster_averages(tissue_dat, tumor_cluster_df)
  CL_clust_avgs <- get_cluster_averages(CCLE_dat, CL_cluster_df)
  
  tissue_subtype_ms <- tissue_dat - tumor_clust_avgs[tumor_cluster_df$seurat_clusters,]
  CCLE_subtype_ms <- CCLE_dat - CL_clust_avgs[CL_cluster_df$seurat_clusters,]
  
  tissue_cov <- cov(tissue_subtype_ms)
  CCLE_cov <- cov(CCLE_subtype_ms)
  
  if(!is.null(pc_dims)) {
    cov_diff_eig <- irlba::prcomp_irlba(tissue_cov - CCLE_cov, n = pc_dims)
  } else {
    cov_diff_eig <- eigen(tissue_cov - CCLE_cov)
  }
  return(cov_diff_eig)
}
